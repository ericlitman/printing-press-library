...
**comments** — Add or edit Linear comments
...
**documents** — View, create, or edit Linear docs
...
**issues** — Get, list, create, or edit Linear issues
...
- Always hit the API. On successful issue create/edit, the HTTP response cache is invalidated AND the changed issue is written back to the local store, so a subsequent `issues list --data-source local` sees the mutation without requiring another sync.
